    ...
    for j in range(1,len(sums)):
        if i != j:
            if sums[i] - sums[j] == 0:
                print(sums[i])
                break
            elif sums[i] - sums[j] in diff.keys():
                if diff[sums[i] - sums[j]][1] > j:
                    diff[sums[i] - sums[j]] = [i, j]
            else:
                diff[sums[i] - sums[j]] = [i, j]

for k in range(0,500*500):
    if k*total_sum in diff.keys():
        print(sums[diff[k*total_sum][0]])
        print("whoops")
        break
